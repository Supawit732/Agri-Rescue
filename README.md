...
ตอนนี้โครงฐานข้อมูลของ Phase 1 อยู่ใน `server/src/db/migrations/001_init.sql` ข้อมูลตัวอย่างถูกอ่านจาก `docs/prototype/App.js` ตอน seed
...
`npm run migrate` สร้างตารางถ้ายังไม่มี และข้ามไฟล์ที่รันไปแล้ว `npm run seed` ใส่พืช ผู้ใช้ แปลง และล็อตจากต้นแบบ โดยข้ามแถวที่มีอยู่แล้ว จึงรันซ้ำได้โดยจำนวนแถวไม่เพิ่ม

รหัสผ่านบัญชีตัวอย่างคือ `demo1234` ชื่อผู้ซื้อ พิกัด และตัวเลขล็อตมาจากต้นแบบ ถ้าต้นแบบไม่มีเบอร์โทร ใช้เบอร์สำรองใน `docs/DECISIONS.md` (D007)
