...
สมัครด้วย `can_sell` / `can_buy` (อย่างน้อยหนึ่งอย่าง) ประเภทสงเคราะห์ตั้ง `charity_approved = 0` จนกว่า `POST /api/auth/admin/approve-charity/:userId` ปฏิเสธด้วย `POST /api/auth/admin/reject-charity/:userId` (ลบโปรไฟล์ที่รอและปิด `can_buy`) รายการรออนุมัติที่ `GET /api/auth/admin/charity-requests` ผู้ใช้เปิดบทบาทอีกฝั่งได้ที่ `PATCH /api/auth/profile` แอปเก็บโหมดขาย/ซื้อใน storage แล้วสลับที่ TopBar บัญชี `is_admin` (เช่น seed 0800000005) เข้าหน้า `/admin` ได้ หน้าคนขับ/ผู้ประสานยังอยู่ใน repo แต่ AuthGate ไม่ส่งผู้ใช้ไปหน้าเหล่านั้น
