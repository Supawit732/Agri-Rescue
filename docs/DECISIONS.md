...
Jest โหลด `server/.env.test` ก่อน แล้วรีเซ็ตตารางข้อมูลใน `agri_rescue_test` ก่อนแต่ละไฟล์ รันทีละไฟล์ และ mock `fetch` ทั้งกรณีสำเร็จและกรณี timeout ไฟล์ `.env.test` อยู่เฉพาะในเครื่องที่รันเทส คัดลอกจาก `server/.env.test.example` แล้วใส่รหัสของฐานเทส ไม่ถูก commit

## D013 — Phase 4: รอบวิ่ง การส่งมอบ และผลกระทบ

จุดที่ส่งให้ `RouteSolver` เรียง pickup ตาม `lot_id` แล้วตามด้วย drop ตาม `buyer_id` ผู้ซื้อคนเดียวมี drop จุดเดียว แม้มีหลายออเดอร์ `driver_id` ของรอบยังเป็นค่าว่าง คนขับทุกคนยืนยันจุดได้

น้ำหนักที่ต่างจาก `weight_kg` เกิน 10% ตั้ง `weight_flag = 1` ส่วนที่เท่ากับ 10% พอดีไม่ตั้งธง ตอน drop รหัสต้องตรงกับ `drop_otp` ของออเดอร์ที่รับของแล้ว ถ้าผู้ซื้อมีหลายออเดอร์ รหัสที่ตรงจะปิดออเดอร์นั้น และจุด drop เสร็จเมื่อออเดอร์ของผู้ซื้อคนนั้นในรอบถูกส่งครบ

`impact_logs.kg_saved` ใช้น้ำหนักที่ชั่งตอน pickup `co2e_kg = kg_saved × 2.5` รายได้เกษตรกรคือ `agreed_price_per_kg × kg_saved` กิโลบริจาคคือน้ำหนักของออเดอร์ที่ `is_donation = 1`

`server.ts` เรียก `expireOpenLots` ตอนเปิดและทุก 10 นาที ล็อตที่หมดอายุแต่สถานะไม่ใช่ `open` ไม่ถูกเปลี่ยน
